...
if __name__ == "__main__":
    ...
    dates_of_arrested = [ "20-4-2015", "10-2-2016"]
    names_of_arrested = [ "", ""]

    noa = StringVar(value=names_of_arrested)

    files_of_arrested = {'20-4-2015':"20-4-2015.pdf", '10-2-2016':"10-2-2016.pdf"}
    ...
    def ShowDetails(*args):
        ...
        if len(itr)==1:
            itr = int(itr[0])
            date = dates_of_arrested[itr]
            name = names_of_arrested[itr]
            pdf = files_of_arrested[date]

            status_msg.set("\"%s\", was last arrested on %s. To Downlad mugshot, \"%s\" Click Save then Go." % (name, date, pdf))
        done_msg.set('')
    ...
    def GoButton(*args):
        ...
        if len(idxs)==1:
            idx = int(idxs[0])
            lbox.see(idx)
            date = dates_of_arrested[idx]
            name = names_of_arrested[idx]
            pdf = files_of_arrested[date]

            usrs_choice = user_ops[uop.get()]

            done_msg.set("Attempting to %s file for the dirtbag named \"%s\"..." % (usrs_choice, name))
    ...
    lbox.bind('<Double-1>', ShowDetails)
